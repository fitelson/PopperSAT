...
describe('constraint_savers', () => {
  const {
    Sentence: { letter, conjunction: and, value: val },
    RealExpr: { given_probability: cpr },
  ...
  const T = val({ value: true })
  const PrAandB = cpr({ arg: AandB, given: T })
  ...
  test('Pr(A & B | ⊤)', () => expect(savers.RealExpr(PrAandB)).toEqual({
    tag: 'record',
    mm_keys: { top: 'RealExpr', inner: 'given_probability' },
    record: {
      arg: {
        tag: 'record',
        mm_keys: { top: 'Sentence', inner: 'conjunction' },
      ...
      given: {
        tag: 'record',
        mm_keys: { top: 'Sentence', inner: 'value' },
        record: {
          value: { tag: 'primitive', value: true }
        },
      },
    },
  }))
...
describe('constraint_loaders', () => {
  ...
  test('Pr(A & B | ⊤)', () => expect(loaders.RealExpr({
    tag: 'record',
    mm_keys: { top: 'RealExpr', inner: 'given_probability' },
    record: {
      arg: {
