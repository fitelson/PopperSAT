...
import { assert_parse_constraint, assert_parse_real_expr, assert_parse_sentence, parse_real_expr } from './parser'
...
describe('parse', () => {
  ...
  describe('RealExpr', () => {
    ...
    // Popper probability requires conditional form Pr(A | B)
    test_parse('Pr(A | ⊤)', cpr(A, val(true)))
    test_parse('Pr(A & B & C | ⊤)', cpr(and(A, and(B, C)), val(true)))
    test_parse('Pr(C | ⊤)^2 / Pr(B | ⊤)^3', divide(power(cpr(C, val(true)), lit(2)), power(cpr(B, val(true)), lit(3))))
    test_parse('Pr(A | B)', cpr(A, B))
    test_parse('Pr(A & B | B → C)', cpr(and(A, B), imp(B, C)))
    test_parse('P(A | true)', cpr(A, val(true)))
    test_parse('P(A & B & C | true)', cpr(and(A, and(B, C)), val(true)))
    test_parse('P(C | true)^2 / P(B | true)^3', divide(power(cpr(C, val(true)), lit(2)), power(cpr(B, val(true)), lit(3))))
    test_parse('P(A | B)', cpr(A, B))
    test_parse('P(A & B | B → C)', cpr(and(A, B), imp(B, C)))
    test_parse('p(A | ⊤)', cpr(A, val(true)))
    test_parse('p(A & B & C | true)', cpr(and(A, and(B, C)), val(true)))
    test_parse('p(C | ⊤)^2 / P(B | true)^3', divide(power(cpr(C, val(true)), lit(2)), power(cpr(B, val(true)), lit(3))))
    test_parse('p(A | B)', cpr(A, B))
    test_parse('p(A & B | B → C)', cpr(and(A, B), imp(B, C)))
    // Bare Pr(A) should fail with helpful error
    test('Pr(A) gives error about conditioning event', () => {
      const result = parse_real_expr('Pr(A)')
      expect(result[0]).toBe(false)
      expect(result[1]).toContain('conditioning event')
    })
    test('P(A) gives error about conditioning event', () => {
      const result = parse_real_expr('P(A)')
      expect(result[0]).toBe(false)
      expect(result[1]).toContain('conditioning event')
    })
  })
