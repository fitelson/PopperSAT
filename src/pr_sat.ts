...
const real_expr_to_gen_string = (expr: RealExpr, s2s: (s: Sentence) => string): string => {
  ...
  const wrap = (expr: RealExpr): string => {
    ...
    if (expr.tag === 'literal' || expr.tag === 'variable' || expr.tag === 'given_probability' || expr.tag === 'negative' || (expr.tag === 'state_variable_sum' && expr.indices.length <= 1)) {
      return sub_str
    } else {
      return `(${sub_str})`
    }
...
export const div0_conditions_in_real_expr = (expr: RealExpr): Constraint[] => {
  if (expr.tag === 'literal') {
    ...
    return [cnot(eq(cpr(expr.given, val(true)), lit(0)))]
  } else if (expr.tag === 'state_variable_sum') {
    return []
  } else if (expr.tag === 'negative') {
