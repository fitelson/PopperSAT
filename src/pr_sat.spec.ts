...
const { variable: vbl, plus: add, multiply: times, minus: subtract, divide: over, state_variable_sum: svs, given_probability: cpr, literal: lit } = PrSatUtils.inits.RealExpr
// Helper: unconditional probability is Pr(X | ⊤) in Popper's system
const pr = (args: { arg: Sentence }): RealExpr => cpr({ arg: args.arg, given: val(true) })
...
describe('TruthTable', () => {
  ...
  test('rejects truth tables large enough to freeze the browser', () => {
    const letters = Array.from({ length: MAX_TRUTH_TABLE_LETTERS + 1 }, (_, index) =>
      letter({ id: 'A', index: index + 1 }))
...
describe('translate', () => {
  describe('3 sentence-letters', () => {
    ...
    test('Pr(A)', () => {
      ...
      const expected = divide(svs({ indices: [0, 1, 2, 3] }), lit({ value: 1 }))
      expect(actual).toEqual(expected)
    })
    test('Pr(B & ~C)', () => {
      const actual = translate_real_expr(tt, pr({ arg: and(B, not(C)) }))
      const expected = divide(svs({ indices: [1, 5] }), lit({ value: 1 }))
      expect(actual).toEqual(expected)
    })
    test('Pr(A | B <=> C)', () => {
    ...
    test('Pr(A & B) = Pr(A) * Pr(B)', () => {
      ...
      const expected = eq(
        divide(svs({ indices: [0, 1] }), lit({ value: 1 })),
        multiply(divide(svs({ indices: [0, 1, 2, 3] }), lit({ value: 1 })), divide(svs({ indices: [0, 1, 4, 5] }), lit({ value: 1 })))
      )
      expect(actual).toEqual(expected)
    })
    test('Pr(A & C) = Pr(A) * Pr(C)', () => {
      ...
      const expected = eq(
        divide(svs({ indices: [0, 2] }), lit({ value: 1 })),
        multiply(divide(svs({ indices: [0, 1, 2, 3] }), lit({ value: 1 })), divide(svs({ indices: [0, 2, 4, 6] }), lit({ value: 1 })))
      )
      expect(actual).toEqual(expected)
    })
    test('Pr(B & C) = Pr(B) * Pr(C)', () => {
      ...
      const expected = eq(
        divide(svs({ indices: [0, 4] }), lit({ value: 1 })),
        multiply(divide(svs({ indices: [0, 1, 4, 5] }), lit({ value: 1 })), divide(svs({ indices: [0, 2, 4, 6] }), lit({ value: 1 })))
      )
      expect(actual).toEqual(expected)
    })
    test('Pr(A & B & C) = Pr(A) * Pr(B) * Pr(C)', () => {
      ...
      const expected = eq(
        divide(svs({ indices: [0] }), lit({ value: 1 })),
        multiply(
          divide(svs({ indices: [0, 1, 2, 3] }), lit({ value: 1 })),
          multiply(divide(svs({ indices: [0, 1, 4, 5] }), lit({ value: 1 })), divide(svs({ indices: [0, 2, 4, 6] }), lit({ value: 1 })))
        )
      )
      expect(actual).toEqual(expected)
